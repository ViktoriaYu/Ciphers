...
inverse_perm = [39, 7, 47, 15, 55, 23, 63, 31,
...
permutation = [15, 6,  19, 20, 28, 11, 27, 16,
               0,  14, 22, 25, 4,  17, 30, 9,
               1,  7,  23, 13, 31, 26, 2,  8,
               18, 12, 29, 5,  21, 10, 3,  24]
 
per_choice_1 = [56, 48, 40, 32, 24, 16, 8,
                0,  57, 49, 41, 33, 25, 17,
                9,  1,  58, 50, 42, 34, 26,
                18, 10, 2,  59, 51, 43, 35,
...
# Преобразует текст из ascii в шестнадцатеричную строку 
def slice_msg(string):
    return [i.zfill(16) for i in wrap(''.join([hex(ord(i))[2:] for i in string]), 16)]
 

# Преобразует строку в 4-битные фрагменты и при необходимости заполняет их нулями
def to_bin(s):
...
def key_gen(block_1, block_2):
    ...
    for i in rotates:
        ...
        li.append(permute(block_1 + block_2, per_choice_2))
...
def des(block, key_array):
    ...
    return wrap(permute(right + left, inverse_perm), 8)
...
def encrypt(msg):
    ...
    for i in slice_msg(msg):
        ...
        permuted_key, permuted_block = permute(
            bin_key, per_choice_1), permute(bin_mess, initial_perm)
...
if __name__ == '__main__':
    ...
    print(" - Your message >>> ", decrypt(encrypted_msg))
